...
int main() {
    ...
    for (int L = L_MIN; L <= L_MAX; L *= 2) {
        ...
        SerialMetropolis simulation(interactionStrength, L, T_MIN, T_MAX, T_STEP, IT);
    ...
    store_performance_to_file(time_results, size, "SerialMetropolis");
    time_results.clear();
    size.clear();
    ...
    // Store performance results for Domain Decomposition
    ...
    for (int L = L_MIN; L <= L_MAX; L *= 2) {
        IT = pow(L, 4.4);
        std::cout << "Simulation start for L = " << L << std::endl;
        SlidingWindow simulation(interactionStrength, L, NUMTHREAD, T_MIN, T_MAX, T_STEP, IT);
    ...
    // Store performance results for Sliding Window
    ...
    for (int L = L_MIN; L <= L_MAX; L *= 2) {
        IT = pow(L, 4.4);
        std::cout << "Simulation start for L = " << L << std::endl;
        CheckBoard simulation(interactionStrength, L, NUMTHREAD / 2, T_MIN, T_MAX, T_STEP, IT);
    ...
    // Store performance results for CheckBoard
    store_performance_to_file(time_results, size, "CheckBoard");
    time_results.clear();
    size.clear();
