...
int main() {
    ...
    int L_MIN, L_MAX;
    float T_MIN, T_MAX, T_STEP, interactionStrength, tolerance;
    long int IT;
    ...
    std::cin >> tolerance;
    //if n not correctly aligned deltaM = -2*n, so then the tolerance will be 2*tolerance
    ...
    for (int L = L_MIN; L <= L_MAX; L *= 2) {
        ...
        AutostopMontecarlo simulation(interactionStrength, L, 2*tolerance, T_MIN, T_MAX, T_STEP);
