...
public:
     ...
     */
    DomainDecomposition(float interactionStrength, int latticeSize, int NUMTHREAD,int MPISIZE, float T_MIN, float T_MAX, float T_STEP, long int IT);
...
protected:
    ...
    /**plus
     * @param end tell if true that we are at the end of the process zone
     */
    void process_border_simulate_step ( std::array<float, 2> prob, std::vector<int>& lattice, int& M, int& E, const int& offset,const bool& end );
    ...
    void exchange_rows(int world_size, int world_rank,std::vector<int>& lattice) ;
